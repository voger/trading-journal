...
The contract is declared in `plugins/contract.py` (the `ImportPlugin` Protocol). A conforming plugin **must** expose: `PLUGIN_NAME`, `DISPLAY_NAME`, `SUPPORTED_EXTENSIONS`, `IMPORT_MODE` (explicit — `contract.IMPORT_MODE_TRADES`/`IMPORT_MODE_EXECUTIONS`, never defaulted), `validate(path) -> (bool, str)`, `parse(path) -> contract.ParseResult(records, balance_events)`, `file_hash(path) -> str`. Optional members (`parse_account_info`, `DEFAULT_ASSET_TYPE`) are reached via contract accessors (`contract.account_info`, `contract.default_asset_type`) — callers never sniff with `hasattr`/`getattr`. Discovery skips non-conforming modules via `contract.conforms`.
